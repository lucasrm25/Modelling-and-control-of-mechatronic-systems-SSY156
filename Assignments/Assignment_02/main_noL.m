...
Y_fun = matlabFunction(Y,'Vars',{g L1 L2 q dq ddq});
...
xidata = zeros(N*n, 1);
for i=1:N
    Ydata(n*i-n+1:n*i,:) = Y_fun(9.81, 0.132, 0.132, data.q(i,2:4)', data.dq(i,2:4)', data.qdd(i,2:4)');
...
unkownvars = [I{1}(2,2); diag(I{2}); diag(I{3}); m2; m3; diag(f); diag(fs)];
